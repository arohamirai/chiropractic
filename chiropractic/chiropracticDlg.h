...
enum drawTpye{ DRAW_LINE = 1, DRAW_DIAGNOSE, DRAW_LINE_MEASURE, DRAW_DIGU_SPX, DRAW_DI1,DRAW_LINE_VERTICALLINE,DRAW_DIGU_LINE,DRAW_CIRCLE,DRAW_YAOZHUI_MEASURE,
	DRAW_BASE_LINE, DRAW_XIONGZHUI_MEASURE, DRAW_JINGZHUIZHENGMIAN_MEASURE, DRAW_JINGZHUIZHANGKOU_MEASURE
};
enum opType {DRAW_RECT = 1, DRAW_MEASURE,DIAG_GP ,DIAG_DG, DIAG_YZ, DIAG_YZJ,DIAG_XZ,DIAG_JZ,DIAG_HZ, DIAG_ZG
};
typedef struct _logLnfo {
	cv::Point p[10];			// 需要记录的关键点
	cv::Point center[5];		// 需要记录的中心点
...
private:
	...
	double m_dLength_wmg_l_qg;			// 左无名骨长度
	double m_dLength_wmg_r_qg;			// 右无名骨长度
